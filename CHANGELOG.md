...
- 会话断点档案与轻量续接:上下文压缩时(PostCompact hook)白捡宿主摘要存档(零成本),会话结束时(SessionEnd hook)用便宜模型从会话记录尾部提炼交接快照;档案集中存 `~/.loki-pet/handoffs/<项目名-哈希>/`(每项目滚动保留 10 份,不写入项目目录);新增 `hand​off.py` 存储模块
- 聊天框 `/resume`:列出各项目最新快照,`/resume 编号` 以"全新会话+注入快照"续接(秒级启动,成本远低于重载旧会话,`resume_cli` 可配),`/resume clear` 清空;项目目录不存在时给出人话提示;完成/失败通知,离开模式发邮件
- 与 `/task` 组成完整断点续传体系:额度断点定时续跑原会话,上下文/关窗断点轻量转世
...
- 计划任务(`/task`):典型场景为订阅额度跑满等重置——`/task 15:00 继续跑测试` 到点自动在最近会话的项目目录执行 `claude --continue -p`(可配 `task_cli`)让宿主恢复对话续跑,完成/失败通知并附输出结尾;`提醒` 开头则仅通知(泡泡+语音+音效);支持 `HH:MM` 与 `+Nh/+Nm` 时间、`/task` 列表、`/task del N` 删除;任务持久化重启不丢,离开模式下同步邮件
- hooks 桥接记录最近活跃会话的工作目录(`last_session.json`),供续跑定位项目

## [1.9.3] - 2026-07-06

### Changed

- 授权等待体验优化:在场等待时长 25 秒 → 5 分钟("没操作就等你");超时回落终端时**静默收起**按钮条,移除"(等太久啦,这条请求回终端处理咯)"打扰性提示;新增兜底过期清理——桥接进程被中断(如终端按 Esc)收不到取消信号时,按钮也会到时自动消失,不留死按钮
