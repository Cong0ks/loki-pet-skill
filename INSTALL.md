...
安装到 `~/.claude/skills/loki-pet/`,Claude Code 等支持 Agent Skills 的工具即可直接调度本宠物。skill 版聊天默认走 `claude -p`(宿主 agent 账号额度),无需 API key;要求宿主 CLI 已登录。详见 README「作为 Agent Skill 使用」。
