...
2. 聊天后端默认 `"backend": "cli"`(见 `config.json`),通过 `cli_command`
   ...
   - 安装在 `.codex/skills/` 下且仍是旧默认 `claude -p` 配置时,启动会自动迁移为
     `codex exec` + `gpt5.4mini`,避免右键菜单仍显示 Claude 模型。
   - 右键菜单"聊天模型(宿主后端)"会按 `cli_command` 识别 Agent 并推荐模型;默认选推荐列表中最省钱的模型。
     Codex 环境默认 `gpt5.4mini`,也可选"跟随宿主默认"。
...
| `cli_model` | cli 聊天模型按宿主 Agent 动态推荐: Claude 默认 `haiku`; Codex 默认 `gpt5.4mini`; Gemini 默认 Flash;空串跟随宿主;右键菜单可切换 |
...
| `pet_width` | 显示宽度(像素) |
| `stop_sound` | Agent 长任务完成时播放的音效路径(空串关闭) |
...
`hook_bridge.py` / `codex_hook_bridge.py` 把 Claude Code / Codex 的权限请求/通知经 `~/.loki-pet/` 文件队列转发给宠物:
弹授权提示时宠物出现 允许 / Yes 15分钟 / 拒绝 / 忽略 按钮,临时授权期间所有命令自动放行,
也可在宠物右键菜单开关"自动帮按 Yes"。

- Claude: hooks 配置在 `~/.claude/settings.json`,细节见 `hook_bridge.py` 文件头注释。
- Codex: 运行 `python install_codex_hooks.py` 合并到 `~/.codex/hooks.json`,
  然后在 Codex 里执行 `/hooks` 审核并 trust 新增 hooks。
- 权限请求会按真实 Agent 显示来源并语音提醒,例如 Codex 环境说"你的 Codex 在找你"。
- Stop hook 只在整轮长任务完成后通知并播放音效,不会在每个小阶段结束时打扰;
  Codex 环境显示"Codex 任务完成啦!"。
